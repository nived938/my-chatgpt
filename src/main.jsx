...
import { Bot, ChevronDown, Copy, Globe, Menu, MessageSquarePlus, Moon, Paperclip, Plus, Search, Send, Settings, Sparkles, Sun, Trash2, X } from 'lucide-react';
import './styles.css';
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      <header className="topbar">
        <button className="icon-btn" onClick={() => setSidebar(true)}><Menu size={20}/></button>
        <div className="model-picker"><Sparkles size={17}/><strong>My ChatGPT</strong><ChevronDown size={15}/></div>
        <div className="top-actions"><button className="icon-btn" onClick={() => setDark(!dark)} title="Theme">{dark ? <Sun size={18}/> : <Moon size={18}/>}</button><button className="icon-btn" onClick={() => setShowSettings(true)}><Settings size={18}/></button></div>
      </header>
      ...
      <section className="conversation">
        ...
        {loading && <div className="message-row assistant"><div className="avatar"><Bot size={17}/></div><div className="bubble thinking"><span></span><span></span><span></span><em>{toolStatus || modelStatus}</em></div></div>}
        <div ref={bottomRef}/>
      </section>

      <form className="composer-wrap" onSubmit={sendMessage}><div className="composer"><button type="button" className="attach" title="Attach files, coming soon"><Paperclip size={19}/></button><textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(e); } }} placeholder="Message My ChatGPT..." rows="1"/><button className="send" disabled={!input.trim() || loading} title="Send"><Send size={18}/></button></div><div className="composer-note">Local model runs in your browser. Web search is used when your question needs fresh information.</div></form>
    </main>
